...
#pragma region Debug
	...
	static bool bWasNum1Pressed = false; //Was pressed keeps its value for the rest of the program even if it leaves the scope
	if(sf::Keyboard::isKeyPressed(sf::Keyboard::Num1))//Check the current state to see if its pressed right now
	{
		bWasNum1Pressed = true;//if it is flag it
	}
	else if(bWasNum1Pressed == true)//if its not currently pressed but it was pressed last...
	{
